...
public class MoviesController {
    ...
    public Response deleteMovie(@PathVariable("id") Long id) {
        ...
        return switch (message) {
            case "eliminado" -> this.responseBuild.success("La película con ID:" + id + " fue eliminada.");
            case "existe_booking" ->
                    this.responseBuild.failed("La pelicula con ID " + id + " se encuentra asociada a un BOOKING");
            case "existe_showtime" ->
                    this.responseBuild.failed("La pelicula con ID " + id + " se encuentra asociada a un SHOWTIME");
            default -> this.responseBuild.failedNotFound("La película con ID:" + id + " no existe.");
        };
    }
